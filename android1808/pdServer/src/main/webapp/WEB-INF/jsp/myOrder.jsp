			...
			<c:forEach items="${orderVOs}" var="orderVO">
				...
				<div id="orderItem">
					<p class="orderItem_title">
						<span id="order_id"> &nbsp;&nbsp;订单编号: <a href="#">${orderVO.ajiaOrder.orderId}</a>
						</span> &nbsp;&nbsp;成交时间：
						<fmt:formatDate value="${orderVO.ajiaOrder.createTime}"
							pattern="yyyy-MM-dd HH:mm" />
						&nbsp;&nbsp; <span> <a href="#" class="servie"> 联系客服<img
								src="../images/myOrder/kefuf.gif" />
						</a> <a href="javascript:void(0)"
							onClick="cancelOrder('${orderVO.ajiaOrder.orderId}')"> 
							取消订单
							</a>
							<c:if test="${orderVO.ajiaOrder.status==1}">
							<a href="/order/toPayment.html?orderId=${orderVO.ajiaOrder.orderId}">
							去付款
							</a>
							</c:if>
						</span>
				...
				<c:forEach items="${orderVO.ajiaOrderItems}" var="ajiaOrderItem">
						...
						<ul>
							<li class="product"><b><a href="#"><img
								...
								class="product_name lf"> <a href="">${ajiaOrderItem.title}</a>
									<br />
							</b> <c:forEach items="${ajiaOrderItem.paramsList}" var="itemParam">
							...
							<li class="trading_status"><c:if
									test="${orderVO.ajiaOrder.status==1}">
                          待付款
                          </c:if> <c:if test="${orderVO.ajiaOrder.status==5}">
                          待收货
                          </c:if> <c:if test="${orderVO.ajiaOrder.status==6}">
                          待评价
                          </c:if> <c:if test="${orderVO.ajiaOrder.status==8}">
                                                         已取消
